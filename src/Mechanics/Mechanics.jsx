    ...
    <View style={styles.phoneRow}>
      ...
      <View style={styles.phoneInputWrap}>
        ...
        <TextInput
          ...
          value={value}
          onChangeText={t => onChange(t.replace(/^0|[^\d]/g, ''))}
        />
      </View>
    </View>
  </View>
...
function MechanicForm({ route }) {
  ...
  useEffect(() => {
    ...
    if (item) {
      ...
      setWhatsapp(item.whatsapp?.replace(/^\+\d{1,3}/, "") || "");
 ...
 const pickMedia = async (type) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("picture", { uri: picture, type: "image/jpeg", name: "m1.jpg" });
      formData.append("picturesec", { uri: picturesec, type: "image/jpeg", name: "m2.jpg" });
      ...
      const url = editItem ? `${baseURL}newmechmain/${editItem.id}` : `${baseURL}newmechmain`;
      ...
      if (response.ok) {
        Alert.alert("Success", editItem ? "Profile updated!" : "Mechanic profile listed for approval!");
        navigation.navigate("repaires");
      }
    } catch (err) {
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <ScrollView contentContainerStyle={styles.scrollBody} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.header}>
              <Text style={styles.mainTitle}>{editItem ? "Edit Profile" : "Join as Mechanic"}</Text>
            ...
            <View style={styles.mediaContainer}>
              <TouchableOpacity style={styles.mediaBox} onPress={() => pickMedia('pic1')}>
                {picture ? (
                  <Image source={{ uri: picture }} style={styles.fullMedia} />
                ) : (
                  <View style={styles.placeholderIcon}>
                    <Ionicons name="build-outline" size={32} color="#ccc" />
                    <Text style={styles.iconLabel}>Work Photo *</Text>
                  </View>
                )}
              ...
              <TouchableOpacity style={styles.mediaBox} onPress={() => pickMedia('pic2')}>
                {picturesec ? (
                  <Image source={{ uri: picturesec }} style={styles.fullMedia} />
                ) : (
                  <View style={styles.placeholderIcon}>
                    <Ionicons name="person-circle-outline" size={32} color="#ccc" />
                    <Text style={styles.iconLabel}>Profile Photo *</Text>
                  </View>
                )}
                <View style={styles.badge}><FontAwesome6 name="plus" size={10} color="white" /></View>
              </TouchableOpacity>
            ...
            <View style={styles.card}>
              ...
              <View style={styles.pickerBox}>
                <Picker selectedValue={category} onValueChange={setCategory}>
                  <Picker.Item label="Select Specialization *" value="" color="#999" />
