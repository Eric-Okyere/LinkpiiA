...
function Rentcarpost({ route }) {
  ...
  const handleSubmit = async () => {
    if (!picture || !name || !phone || !category || !price || !drivername) {
      setError("Please fill all required fields (*) including Driver Name");
      return;
    }
    setIsLoading(true);
    ...
    try {
      ...
      formData.append("picture", { uri: picture, type: "image/jpeg", name: "c1.jpg" });
      formData.append("picturesec", { uri: picturesec, type: "image/jpeg", name: "c2.jpg" });
      ...
      const url = editItem ? `${baseURL}rentcar/${editItem.id}` : `${baseURL}rentcar`;
      ...
      if (response.ok) {
        Alert.alert("Success", editItem ? "Updated!" : "Car listed for approval!");
        navigation.navigate("rentdash");
      }
    } catch (err) {
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <ScrollView contentContainerStyle={styles.scrollBody} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.header}>
              <Text style={styles.mainTitle}>{editItem ? "Edit Rental" : "Rent Out Car"}</Text>
              <Text style={styles.subTitle}>List your vehicle for short or long term rental</Text>
            </View>

            {/* MEDIA SECTION */}
            <View style={styles.mediaContainer}>
              {[ {uri: picture, type: 'pic1', icon: 'car-sport-outline'}, 
                ...
                <TouchableOpacity key={i} style={styles.mediaBox} onPress={() => pickMedia(item.type)}>
                  {item.uri ? (
                    ...
                    <View style={styles.placeholderIcon}>
                      ...
                      <Text style={styles.iconLabel}>{i === 0 ? "Main View" : "Interior/Side"}</Text>
                    </View>
                  )}
            ...
            <View style={styles.card}>
              ...
              <TextInput style={styles.input} placeholder="Car Model & Year (e.g. Toyota Camry 2022) *" value={name} onChangeText={setName} />
              <TextInput style={styles.input} placeholder="Price per Day (₵) *" keyboardType="numeric" value={price} onChangeText={setPrice} />
              
              <View style={styles.pickerBox}>
                <Picker selectedValue={category} onValueChange={setCategory}>
                  <Picker.Item label="Select Car Category *" value="" color="#999" />
