...
const EquipmentMana = (props) => {
  ...
  useFocusEffect(
    useCallback(() => {
      setLoading(true);
      axios.get(`${baseURL}equipmentmain/user/${myProducts.user}`)
        .then((res) => {
  ...
  const deleteProducts = async (id) => {
    try {
      const response = await fetch(`${baseURL}equipmentmain/${id}/deactivate`, { method: 'PUT' });
      if (response.ok) {
  ...
  const ListHeader = () => (
    <View style={styles.listHeader}>
      <Text style={[styles.headerText, { width: width * 0.25 }]}>Equipment</Text>
      ...
      <Feather name="bar-chart" size={16} color="#888" style={{ flex: 1, textAlign: 'center' }} />
    </View>
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTopRow}>
          ...
          <Text style={styles.headerTitle}>Equipment Asset Manager</Text>
        </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <EquipList {...item} navigation={navigation} delete={deleteProducts} index={index} />
          )}
        ...
        <View style={styles.emptyContainer}>
          <Feather name="tool" size={70} color="#ddd" />
          <Text style={styles.emptyText}>No equipment assets listed yet.</Text>
          <TouchableOpacity 
            style={styles.addBtn}
            onPress={() => navigation.navigate("equipost")}
          >
            <Text style={styles.addBtnText}>Post Equipment</Text>
          </TouchableOpacity>
        </View>
      )}
    </SafeAreaView>
