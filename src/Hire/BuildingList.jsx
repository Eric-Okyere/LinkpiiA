...
const BuildingList = (props) => {
  ...
  return (
    <View style={styles.rowWrapper}>
      ...
      <Modal animationType='fade' transparent visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setModalVisible(false)}>
          <View style={styles.modalContent}>
            <Text style={styles.modalHeader}>Property Options</Text>
            <View style={styles.actionGrid}>
              <TouchableOpacity 
                style={styles.actionBox} 
                onPress={() => { props.navigation.navigate("postbuilding", { item: props }); setModalVisible(false); }}
              >
                <View style={[styles.iconContainer, { backgroundColor: '#e3f2fd' }]}>
                  <MaterialCommunityIcons name="home-edit" size={28} color="#1976d2" />
                </View>
      ...
      {/* --- Property Card --- */}
      <TouchableOpacity
        onPress={() => props.navigation.navigate('sigview', { ...props })}
      ...
      >
        ...
        <View style={[styles.cell, { width: width * 0.3 }]}>
...
const styles = StyleSheet.create({
  rowWrapper: { backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: '#f0f0f0', marginBottom:20 },
  card: { flexDirection: "row", alignItems: 'center', padding: 15 },
  cell: { justifyContent: 'center' },
  img: { height: 44, width: 44, borderRadius: 12, backgroundColor: '#f0f0f0' },
  stackedImg: { marginLeft: -20, marginTop: 12, borderWidth: 2, borderColor: '#fff' },
  nameText: { fontSize: 14, fontWeight: '700', color: '#2c3e50' },
  townText: { fontSize: 11, color: '#95a5a6', marginTop: 2 },
  priceText: { fontSize: 13, fontWeight: 'bold', color: '#27ae60' },
  statusBadge: { paddingHorizontal: 6, paddingVertical: 3, borderRadius: 5, alignSelf: 'flex-start' },
