...
// --- Modern Media Component ---
const CarouselMedia = ({ item, isActive }) => {
  if (item.type === 'video') {
    return <VideoContent uri={item.uri} isActive={isActive} />;
  }
  return <Image source={{ uri: item.uri }} style={styles.carouselMedia} resizeMode="cover" />;
};

const VideoContent = ({ uri, isActive }) => {
      ...
      >
        {data.map((item, idx) => (
          <View key={idx} style={{ width }}>
            <CarouselMedia item={item} isActive={currentIndex === idx} />
          </View>
        ))}
      </ScrollView>
...
function RentCarAdminSig({ route }) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.navigate("rentdash")} style={styles.backBtn}>
            <Ionicons name="arrow-back" size={24} color="black" />
          <Text style={styles.headerTitle}>Car Rental Details</Text>
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.content}>
          <View style={styles.titleRow}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.categoryLabel}>Verified Rental</Text>
            </View>
          ...
          <Text style={styles.sectionTitle}>Owner / Agent Contacts</Text>
          ...
          <Text style={styles.sectionTitle}>Car Description</Text>
          ...
          <View style={styles.locationBox}>
            ...
            <View style={styles.locRow}>
              ...
              <Text style={styles.locText}> Exact Location: <Text style={{fontWeight:'700'}}>{item.location}</Text></Text>
            </View>
          </View>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#FFF', bottom: 20, paddingBottom:30 },
  header: { padding: 15, borderBottomWidth: 1, borderBottomColor: '#EEE', flexDirection: 'row', alignItems: 'center' },
  backBtn: { flexDirection: 'row', alignItems: 'center' },
  headerTitle: { marginLeft: 15, fontWeight: '700', fontSize: 16, color: '#333' },
...
export default RentCarAdminSig;